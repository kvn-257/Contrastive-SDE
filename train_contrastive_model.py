...
from guided_diffusion.train_util import NT_Xent
...
def main(args):
    ...
    def forward_backward_log(data_loader, prefix="train"):
        ...
        shape = (args.batch_size, 3, batch.shape[2], batch.shape[3])
        down_N = 32
        shape_d = (
            args.batch_size, 3, int(batch.shape[2] / down_N), int(batch.shape[2] / down_N))
        down = Resizer(shape, 1 / down_N).to(device)
        up = Resizer(shape_d, down_N).to(device)
        
        criterion = NT_Xent(args.batch_size, temperature = 0.5, world_size = 1)
        ...
        for i, (sub_batch, sub_labels, sub_t) in enumerate(
            ...
            img_lf = up(down(sub_batch))

            z_i, z_j = model(sub_batch, img_lf, timesteps=sub_t)
...
if __name__ == "__main__":
    dataset = 'cat2dog' #cat2dog/wild2dog/male2female(mutli-domain)
    #defalut args
